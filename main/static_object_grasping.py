...
def dmp_setup():
    ...
    y_demo[1, :] += 2 * np.sin(2 * np.pi * t)
    y_demo[2, :] += 2 * np.cos(2 * np.pi * t)
...
def real_time_trajectory_generation(dobot, dmp, predicted_arm_pos_queue, object_catched):
    ...
    while not object_catched:
        try:
            ...
            predicted_arm_pos = predicted_arm_pos_queue.get()
            while True:
                try:
        ...
        while True:
            ...
            if not object_catched:
                ...
                if 640 > cx > 0 and 480 > cy > 0:
                    ...
                    # cv2.circle(color_image, (predicted_x, predicted_y), 10, (255, 0, 0), -2.txt)
                    ...
                    if pre_predicted_arm_pos is not None:
                        if (abs(pre_predicted_arm_pos[0] - predicted_arm_pos[0]) > 10 or abs(pre_predicted_arm_pos[1] - predicted_arm_pos[1]) > 10
                                or abs(pre_predicted_arm_pos[2] - predicted_arm_pos[2]) > 10):
                    ...
                    if reaching_state:
                        ...
                        dobot.suction_cup_suck()
                        time.sleep(2)
                        # print("Move the object to point (200, 0, 50)")
                        dobot.move_dobot_arm_wait([200, 0, 50])
                        dobot.move_dobot_arm_wait([250, 0, -40])
                        dobot.suction_cup_release()
                        dobot.move_dobot_arm_wait([200, 0, 50])
